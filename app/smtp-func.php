...
function smtpmail($mail_to, $subject, $message, $headers='') {
        ...
        // Настройки почты
        $config['smtp_username'] = 'admin@racks-montazh.ru';  //Смените на имя своего почтового ящика из ISPManager.
        $config['smtp_password'] = '1988678';  //Измените пароль.
        ...
        $SEND .=  $message."\r\n";
         if( !$socket = fsockopen($config['smtp_host'], $config['smtp_port'], $errno, $errstr, 30) ) {
            ...
            if (!server_parse($socket, "250", __LINE__)) {
               if ($config['smtp_debug']) echo '<p>е могу отправить команду MAIL FROM:</p>';
               fclose($socket);
               return false;
            }
